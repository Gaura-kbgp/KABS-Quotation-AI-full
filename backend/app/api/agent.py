...
async def rescan_project(payload: dict):
    ...
    try:
        ...
        # Run rule-based local extraction
        result = extract_rooms_rule_based(pdf_bytes)
        rooms = result.get("rooms", [])

        if not rooms:
            # Fallback to AI extraction
            result2 = await analyze_drawing_local_inner(pdf_url)
            rooms = result2.get("rooms", [])

        extracted_data = {
            "rooms": rooms,
            "smart_agent_explanations": [f"Re-scanned via rescan-project endpoint. Method: {result.get('method', 'rule-based-local')}"]
        }
        ...
        print(f"[rescan] Project {project_id} re-scanned. {len(rooms)} rooms found.")
        return {"success": True, "rooms_count": len(rooms), "rooms": rooms}
